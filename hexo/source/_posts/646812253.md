...
- `value`用`count`取出最后一个状态。</br> 
- `push`用`count`取出最后一个状态的`index`，取出对应的`type_list`，然后对这个`type_list`进行操作。通过实例化一个新的模板，添加一个新的状态，并且存放这个新的`type_list`。</br> 
- `pop`与`push`类似，只不过对`type_list`的操作变成了删除。</br> 
