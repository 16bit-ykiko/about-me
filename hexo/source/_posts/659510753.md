...
在 C 语言中，我们经常通过这样计算 offset 的方式来访问结构体成员。如果把它封装成函数，还能用来根据传入的参数动态访问结构体的成员变量。然而上面的代码在 C++ 中是 undefined behavior，具体的原因可以参考 [Stack Overflow](https:https%3A//stackoverflow.com/questions/66800315/can-i-manually-access-fields-by-their-raw-offset-in-c) 上的这个讨论。但是如果我们确实有这样需求，那该怎么合法的实现需求呢？C++ 为我们提供了一层抽象：[pointers to members](https:https%3A//en.cppreference.com/w/cpp/language/pointer%23Pointers_to_members)，用来合法进行这样的操作。
...
一个指向非静态成员函数`f`的成员指针可以用`&C::f`进行初始化。由于不能对非静态成员函数取地址，`&(C::f)`和`&f`什么都不表示。类似的可以通过运算符`.*`和`->*`来访问对应的成员函数。如果成员函数是重载函数，想要获取对应的成员函数指针，请参考 [如何获取重载函数的地址](https:https%3A//en.cppreference.com/w/cpp/language/overloaded_address)。示例代码如下
...
对于三大主流编译器，GCC 遵循 Itanium C++ ABI ，MSVC 则遵守 MSVC C++ ABI，Clang 通过不同的编译选项可以分别设置为这两种 ABI。关于 ABI 的详细讨论请移步 [彻底理解 C++ ABI](https://www.ykiko.me/692886292) 和 [MSVC 与 GCC 产生的动态库如何才能相互替换](https://www.zhihu.com/question/653778109/answer/3480007666)，这里不过多介绍。

- [Itanium ABI](https:https%3A//itanium-cxx-abi.github.io/cxx-abi/abi.html%23data-member-pointers) 具有公开的文档，之后的相关描述主要参考这个文档
- MSVC ABI 没有公开的文档，之后的相关描述主要参考 [MSVC C++ ABI Member Function Pointers](https:https%3A//rants.vastheman.com/2021/09/21/msvc/) 这篇博客
...
在 C++23 之后，如果使用 [explicit this](https:https%3A//en.cppreference.com/w/cpp/language/member_functions%23Explicit_object_member_functions) 定义成员函数，则`&C::f`可以直接获取对应成员函数的函数指针，不需要像上面那样多一层包裹了 
