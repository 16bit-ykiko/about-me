...
updated: 2024-04-10 01:27:09
type: 'post'
cover: 'https://pica.zhimg.com/v2-0a5f7ba961aa8a8a2358511b09d2d6c6_720w.jpg?source=172ae18b'
---
...
如果你只对代码感兴趣，我已经将相关的代码放在 [Compiler Explorer - C++](https://godbolt.org/z/fK7v8snTn) 。三大编译器 C++20 均编译通过，你可以直接看到编译器的输出结果。防止链接失效，也放到 [GitHub](https://github.com/16bit-ykiko/blog/blob/main/code/compile-time-counter.cpp) 上。如果你想要了解它的原理，欢迎继续往下阅读。C++ 标准极其复杂，作者也没法保证文章内容百分百正确，如果有任何错误，欢迎评论区讨论。
...
{
    A a;
    touch(a); // 读者可以自己尝试一下，打印出 member 的值
}
```
...
虽然上面实现了一位的加法，但是并不能满足我们的需求，我们可以直接实现一个编译期的程序计数器！
...
> [Revisiting Stateful Metaprogramming in C++20](https://mc-deltat.github.io/articles/stateful-metaprogramming-cpp20)[b.atch: Non-constant constant-expressions in C++](https://b.atch.se/posts/non-constant-constant-expressions/)[How to Hack C++ with Templates and Friends](https://ledas.com/post/857-how-to-hack-c-with-templates-and-friends/)