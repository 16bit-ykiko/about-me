...
updated: 2024-02-21 15:19:53
type: 'post'
cover: 'https://pic1.zhimg.com/v2-adf8ecb0727654afd66f21ef205f6288_720w.jpg?source=172ae18b'
---
# 为什么说 C/C++ 编译器不保留元信息？
...
结构体的大小一般可以由特定规则算出从它的成员算出，往往要考虑内存对齐，而且是编译器决定的。例如 [msvc](https://learn.microsoft.com/en-us/cpp/c-language/storage-and-alignment-of-structures%3Fview%3Dmsvc-170)。但总之在编译的时候结构体的大小就是已知的了，我们也可以通过`sizeof`获取类型或者变量的大小。那么这里的`Point point`变量定义就很好理解，类型大小已知，相对于在栈上分配了一块内存。
