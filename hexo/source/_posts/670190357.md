...
结构体的大小一般可以由特定规则算出从它的成员算出，往往要考虑内存对齐，而且是编译器决定的。例如 [msvc](https:https%3A//learn.microsoft.com/en-us/cpp/c-language/storage-and-alignment-of-structures%3Fview%3Dmsvc-170)。但总之在编译的时候结构体的大小就是已知的了，我们也可以通过`sizeof`获取类型或者变量的大小。那么这里的`Point point`变量定义就很好理解，类型大小已知，相对于在栈上分配了一块内存。
