...
一个 ABI（应用二进制接口）是一种标准，它定义了高级语言中的低级概念与特定硬件/操作系统平台的机器码能力之间的映射关系。其中包括以下内容：  

- C/C++/Fortran/...  数据类型在内存中的布局方式（数据大小/对齐方式） 
- 嵌套函数调用的工作方式（如何存储如何返回到函数调用者的信息，CPU 寄存器和/或内存中传递函数参数的位置和方式） 
- 程序启动/初始化的工作方式（可执行文件具有什么样的数据格式，代码/数据如何从中加载，DLL 的工作方式等）  


对于这些问题的答案是：  

- 特定于编程语言（因此你有 C ABI、C++ ABI、Fortran ABI、Pascal ABI、... 甚至 Java 字节码规范，尽管针对的是虚拟处理器而不是真实硬件，但也是一种 ABI） 
- 特定于操作系统（同一硬件上的 MS Windows 和 Linux 使用不同的 ABI） 
- 特定于硬件/CPU（ARM 和 x64 ABI 是不同的） 
- 长时间演变（现有的 ABI 经常被更新/修订，以便利用新的 CPU 特性，比如，规定应用程序如何使用 x86 SSE 寄存器当然只有在 CPU 有了这些寄存器之后才可能，因此现有的 ABI 需要澄清）  


如果没有某种标准化，由不同编译器创建的（机器）代码将无法使用相同类型的库（你怎么知道库代码期望函数参数或数据结构以何种方式传递？）。  每个平台（特定硬件、操作系统软件和用特定编程语言编写/用特定编译器编译的代码的组合）都定义了一整套 ABI，以实现互操作性。

在这个领域的术语并不清晰，有时人们只是谈论 ABI，其他时候称之为 平台补充，或者提及编程语言并说例 C++ ABI。请记住，并不存在这样的一个东西。  你在问题中提到的文件都是这种（语言/操作系统/硬件特定的 ABI）的具体示例。  

即使在特定平台上，也没有必要只有一个 ABI（集） ，因为不同的约定可能具有不同的优点（因此可能提供更好的性能/更小的代码/更好的内存使用率/... - 取决于程序），系统设计者通常试图灵活/宽容。例如，在 32 位 Microsoft Windows 上，函数调用约定部分有多种 ABI（fastcall、stdcall、pascal 等）。  
